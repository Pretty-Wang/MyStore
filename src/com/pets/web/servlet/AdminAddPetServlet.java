...
//			 如果出现这个异步，说明单个文件超出了80KB
		 ...
		 * 4.1 首先把“普通表单字段”放到一个Map中，再把Map转换成Pet和Category对象，再建立两者的关系
		 */
		...
		Pet pet = CommonUtils.toBean(map, Pet.class);//把Map中大部分数据封装到Pet对象中
...
//		 保存上传的图片，把图片new成图片对象：Image、Icon、ImageIcon、BufferedImage、ImageIO
		 ...
		 * 1. 获取真实路径
		 */
		String savepath = this.getServletContext().getRealPath("/pet_img");
		/*
		 * 2. 创建目标文件
		 */
...
//		 把图片的路径设置给pet对象
		pet.setImage_w("pet_img/" + filename);
...
//		filename = CommonUtils.uuid() + "_" + filename;
		 ...
		 * 1. 获取真实路径
		 */
		savepath = this.getServletContext().getRealPath("/pet_img");
		/*
		 * 2. 创建目标文件
		 */
		...
		pet.setImage_b("pet_img/" + filename);
		
		
		
		
		// 调用service完成保存
		pet.setBid(CommonUtils.uuid());
	...
	private void error(String msg, HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/adminjsps/admin/pet/add.jsp").
